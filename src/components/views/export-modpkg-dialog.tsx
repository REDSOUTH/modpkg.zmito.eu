...
export function ExportModpkgDialog({ isOpen, onClose }: ExportModpkgDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !isExportingZip && !open && onClose()}>
      <DialogContent
        hideClose
        className="w-[95vw] max-w-[850px] bg-[#0A0A0A] border border-[#1E1E1E] p-0 gap-0 overflow-hidden shadow-2xl rounded-2xl text-white"
      >
        {/* Header matching Create New MODPKG with centered large custom close button */}
        <DialogHeader className="p-5 px-6 border-b border-[#1E1E1E] flex flex-row items-center justify-between space-y-0">
          <div className="flex items-center gap-3">
            <Package className="w-8 h-8 text-[#FE5000] shrink-0" />
            <div>
              <DialogTitle className="text-xl font-bold text-white">Export MODPKG</DialogTitle>
          ...
          <DialogClose asChild disabled={isExportingZip}>
            <button
              title="Close"
              className="w-9 h-9 rounded-xl flex items-center justify-center text-white hover:bg-[#1E1E1E] transition-colors cursor-pointer"
            >
        ...
        <ScrollArea className="max-h-[82vh] overflow-y-auto">
          <div className="p-6 flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-3 p-4 bg-[#141414] rounded-xl">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs px-2.5 py-0.5 rounded-full bg-white/5 text-white/70 font-medium">
                    {totalContentCount} items
                  </span>
                  {totalCustomFilesCount > 0 && (
                    <span className="text-xs px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-300 flex items-center gap-1 font-medium">
                  ...
                  <div className="flex flex-wrap gap-1.5 max-h-36 overflow-y-auto pr-1">
                    {installedContent.map((item) => (
                      <Tooltip key={item.id}>
                        <TooltipTrigger asChild>
                          <div className="w-9 h-9 rounded-lg bg-[#1C1C1C] border border-white/10 flex items-center justify-center relative overflow-hidden cursor-pointer shrink-0">
                            {item.iconUrl && item.iconUrl !== "/logo.svg" ? (
                              ...
                              <ContentTypeIcon type={item.contentType} iconClassName="w-4 h-4 text-white/60" />
                            )}
                        ...
                        <TooltipContent side="top" className="bg-[#1C1C1C] text-white border-[#333] text-xs p-2.5 shadow-xl max-w-xs">
                    ...
                    {customFiles.map((file) => (
                      <Tooltip key={file.id}>
                        ...
                        <TooltipContent side="top" className="bg-[#1C1C1C] text-white border-[#333] text-xs p-2.5 shadow-xl">
                          <p className="font-semibold text-amber-300">{file.name}</p>
                          <p className="text-[11px] text-white/50 mt-0.5">Target: {file.targetPath}</p>
                          <p className="text-[11px] text-white/40 capitalize">Type: {file.type}</p>
                        </TooltipContent>
                      </Tooltip>
                    ))}
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col justify-between p-4 bg-[#141414] rounded-xl gap-4">
                <div className="flex flex-col text-left gap-1.5">
                  <div className="flex items-center gap-2">
                    <FileJson className="w-4 h-4 text-sky-400 shrink-0" />
                    <h3 className="text-sm font-semibold text-white">Version Index Manifest</h3>
                  </div>
                  <p className="text-xs text-white/50 leading-relaxed">
                    Exports <code className="text-white/80 bg-white/5 px-1 py-0.5 rounded text-[11px]">{safeId}.mpkg.json</code> with the current release manifest, dependencies and overrides.
                  </p>
                </div>
                <Button
                  ...
                  variant="outline"
                  className="w-full bg-[#1C1C1C] hover:bg-[#252525] border-0 text-white text-xs h-9 rounded-lg font-medium transition-all flex items-center justify-center gap-2"
                >
              ...
              <div className="flex flex-col justify-between p-4 bg-[#141414] rounded-xl gap-4">
                <div className="flex flex-col text-left gap-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-semibold text-white">Full Project File</h3>
                  </div>
                  <p className="text-xs text-white/50 leading-relaxed">
                    Exports <code className="text-white/80 bg-white/5 px-1 py-0.5 rounded text-[11px]">{safeId}.mpkg-proj.json</code> containing full project data, releases history and settings.
                  </p>
                </div>
                <Button
            ...
            <div className="flex flex-col p-5 bg-[#141414] border-2 border-[#FE5000] rounded-xl gap-5 relative overflow-hidden">
              <div className="flex items-start justify-between gap-4">
                <div className="flex flex-col text-left gap-1.5">
                  <div className="flex items-center gap-2">
                    <Archive className="w-5 h-5 text-[#FE5000] shrink-0" />
                    <h3 className="text-sm font-bold text-white">Complete Package ZIP (.mpkg.zip)</h3>
                    <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded bg-[#FE5000] text-white">
                      Full Bundle
                    </span>
                  </div>
                  <p className="text-xs text-white/60 leading-relaxed">
                    Builds a complete offline archive with all jars, resourcepacks, shaders and overrides placed into their respective directories (<code className="text-white/80 text-[11px]">/mods/</code>, <code className="text-white/80 text-[11px]">/resourcepacks/</code>, <code className="text-white/80 text-[11px]">/config/</code>).
                  </p>
                </div>
              </div>

              {/* ZIP Inclusion Selects (no outer border) */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-black/40 rounded-xl">
                ...
                <div className="flex flex-col gap-1.5 text-left">
                  ...
                  <Select
                    ...
                    disabled={isExportingZip}
                  >
                    <SelectTrigger className="bg-[#1E1E1E] border-2 border-[#1E1E1E] text-white focus:ring-0 focus:border-[#FE5000] text-xs h-9 rounded-xl">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent className="bg-[#0A0A0A] border-2 border-[#1E1E1E] text-white rounded-xl">
                      <SelectItem value="yes" className="text-xs focus:bg-[#1E1E1E] focus:text-[#FE5000]">
                        Yes, include in ZIP (Recommended)
                      </SelectItem>
                      <SelectItem value="no" className="text-xs focus:bg-[#1E1E1E] focus:text-[#FE5000]">
                        No
                      </SelectItem>
                    </SelectContent>
                  </Select>
              ...
              {isExportingZip && zipProgress && (
                <div className="flex flex-col gap-2.5 p-4 bg-black/60 rounded-xl">
                  ...
                  <Progress value={zipProgress.percentage} className="h-2 bg-[#222]" />
                </div>
              )}
              ...
              {failedItems.length > 0 && (
                <div className="flex flex-col gap-2 p-3 bg-amber-500/10 rounded-xl text-left">
                  ...
                  <div className="max-h-24 overflow-y-auto flex flex-col gap-1 pl-6 text-[11px] text-white/60">
                    {failedItems.map((fi, idx) => (
                      <div key={idx} className="flex items-center justify-between gap-2">
                        <span className="text-white/80 truncate">• {fi.name} ({fi.provider})</span>
                        <span className="text-white/40 shrink-0">{fi.reason}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {exportCompleted && failedItems.length === 0 && (
                <div className="flex items-center gap-2 p-3 bg-emerald-500/10 rounded-xl text-emerald-400 text-xs font-semibold">
